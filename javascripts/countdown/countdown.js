$(function()
{
    var countDownClock = 
    {
		...
		{  
		    ...
		    {
		        daysCount = ('0000' + (daysCount - 1)).slice(-4);
		        hoursCount = ('00' + (24 - hoursCount)).slice(-2); 
		    }
		    if (date[4] - currentDate.getMinutes() < 0)
		    {
		        hoursCount = ('00' + (hoursCount - 1)).slice(-2);
		        minuteCount = ('00' + (60 - minuteCount)).slice(-2); 
		        if (hoursCount < 0)
		        {
    		        daysCount = ('0000' + (daysCount - 1)).slice(-4);
		    ...
		    {
		        minuteCount = ('00' + (minuteCount - 1)).slice(-2);
		        secondCount = ('00' + (60 - secondCount)).slice(-2); 
		        if (minuteCount < 0)
		        {
    		        hoursCount = ('00' + (hoursCount - 1)).slice(-2);
